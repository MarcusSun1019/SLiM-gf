...
$HOME/software/slim-install/bin/slim -s ${SEED_NUM} -d modelindex=$MODELINDEX -d m=${MODEL_NUM[0]} -d direction=${MODEL_NUM[1]} -d gf_time_frac=${MODEL_NUM[2]} -d r=${MODEL_NUM[3]} $TESTDIR/slim/script.slim
