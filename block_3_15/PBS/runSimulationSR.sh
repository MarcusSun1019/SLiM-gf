...
$HOME/software/slim-install/bin/slim -s ${SEED_NUM} -d modelindex=$MODELINDEX -d delta_t1=${MODEL_NUM[0]} -d Ne1=${MODEL_NUM[1]} -d Ne2=${MODEL_NUM[2]} -d Ne3=${MODEL_NUM[3]} -d sample_size=${MODEL_NUM[4]} -d m=${MODEL_NUM[5]} -d r=${MODEL_NUM[6]} -d mut=${MODEL_NUM[7]} $TESTDIR/slim/script.slim
